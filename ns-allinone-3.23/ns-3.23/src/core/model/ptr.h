...
#define PTR_DEBUG_PRINT_PACKET_ENABLED 0

#if PTR_DEBUG_PRINT_PACKET_ENABLED
#include <typeinfo>
#include <execinfo.h>
#include <stdio.h>
#include <stdlib.h>
...
#if PTR_DEBUG_PRINT_PACKET_ENABLED
      std::string s1 (typeid (*m_ptr).name ());
      std::string s2 ("Packet");
      if (s1.find (s2) != std::string::npos)
        {
          std::cout<< "Ptr<" << s1<<">::Acquire " << m_ptr << " refcount=" << m_ptr->GetReferenceCount () << "\n";
  ...
  if (m_ptr != 0) 
    {
      ...
      if (s1.find (s2) != std::string::npos)
        {
          std::cout<< "Ptr<" << s1<<">::~Ptr " << m_ptr << " refcount=" << m_ptr->GetReferenceCount () - 1 << "\n";
  ...
  if (m_ptr != 0) 
    {
      ...
      if (s1.find (s2) != std::string::npos)
        {
          std::cout<< "Ptr<" << s1<<">::operator= " << m_ptr << " refcount=" << m_ptr->GetReferenceCount () - 1<< "\n";
