 ...
 * \ingroup examples
 * 
 * hw-netlink-couwbat is a wrapper for the Couwbat module which allows it to run standalone and communicate with the final AED PHY kernel module that controls hardware
 */
...
enum {
        CW_CMD_WIFI_EXTRA_UNSPEC        = 0x00,             /**< reserved, do not edit */
        // Following already defined in couwbat-meta-header.h
...
//        CW_CMD_WIFI_EXTRA_NO_SEQ        = 0x07,             /**< Sequence aborted, no sequence info */
        CW_CMD_DEBUG_DMA_START          = 0x08,             /**< DEBUG Command: Write to DMA */
        CW_CMD_DEBUG_DMA_STOP           = 0x09,             /**< DEBUG Command: Write to DMA */
        CW_CMD_DEBUG_DMA_WRITE          = 0x0A,             /**< DEBUG Command: Write to DMA */
        CW_CMD_DEBUG_DMA_READ           = 0x0B,             /**< DEBUG Command: Read from DMA */
        CW_CMD_DEBUG_DMA_STATUS         = 0x0C,             /**< DEBUG Command: Read DMA Status */
        CW_CMD_DEBUG_DMA_RESET          = 0x0D,             /**< DEBUG Command: Reset DMA */

        /* add new receptions status here */                /* only on '*' to avoid doxygen documentation*/
        __CW_CMD_WIFI_EXTRA_AFTER_LAST,                 /**< Arbitrary command, framing the netlink commands, do not edit*/
        __CW_CMD_WIFI_EXTRA_MAX = __CW_CMD_WIFI_EXTRA_AFTER_LAST -1 /**<Arbitrary command, framing the netlink commands, do not edit */
};
#define CW_CMD_WIFI_EXTRA (__CW_CMD_WIFI_EXTRA_MAX)         /**< Framing reception status and the netlink command enum*/

enum {
        CW_CMD_ATTR_UNSPEC = 0,                 /**< reserved, do not edit */
        CW_CMD_ATTR_FLAGS,                      /**< Receptions status flags attribute */
        CW_CMD_ATTR_FREQUENCY_BAND,             /**< Sub Band attribute, (0-1|1-2|2-3)Ghz */
        CW_CMD_ATTR_OFDM_SYM_SFRAME_COUNT,      /**< Absolute super frame count attribute */
        CW_CMD_ATTR_OFDM_SYM_OFFSET,            /**< Relative start time of MPDU attribute */
        CW_CMD_ATTR_OFDM_SYM_LEN,               /**< Duration (no. OFDMS symbols) of MPDU attribute */
        CW_CMD_ATTR_ALLOCATED_SUB_CHANNELS,     /**< Bitarry of used logical sub channels attribute */
        CW_CMD_ATTR_MCS,                        /**< MSC/Bitrate for each sub channel attribute */
        CW_CMD_ATTR_CQI,                        /**< Channel qualitiy for each sub channel attribute */
        CW_CMD_ATTR_RESERVED,                   /**< (reserved) attribute */
        CW_CMD_ATTR_I_FC,                       /**< Frame control attribute */
        CW_CMD_ATTR_I_SRC,                      /**< Source address attribute */
        CW_CMD_ATTR_I_DST,                      /**< Destination address attribute */
        CW_CMD_ATTR_I_SEQ,                      /**< Sequence Number attribute */
        CW_CMD_ATTR_PAYLOAD,                    /**< Payload attribute, the payload itself*/
        CW_CMD_ATTR_DEBUG_STRING,               /**< Debug attribute, null terminated string */
        __CW_CMD_ATTR_MAX,                      /**< reserved, do not edit */
};
...
static void init_cw_att_genl_policy()
{
  ...
  temp.type = NLA_U64;
  cw_att_genl_policy[CW_CMD_ATTR_ALLOCATED_SUB_CHANNELS] = temp;
  temp.type = NLA_U32;
  ...
  cw_att_genl_policy[CW_CMD_ATTR_I_SEQ] = temp;
  temp.type = NLA_U8;
  cw_att_genl_policy[CW_CMD_ATTR_MCS] = temp;
  cw_att_genl_policy[CW_CMD_ATTR_CQI] = temp;
  cw_att_genl_policy[CW_CMD_ATTR_I_FC] = temp;
  cw_att_genl_policy[CW_CMD_ATTR_I_SRC] = temp;
  cw_att_genl_policy[CW_CMD_ATTR_I_DST] = temp;
...
int main (int argc, char *argv[])
{
  ...
  while (1)
    {
      ////////////////////
      // Due to pack of SF_START from PHY, generate SF_START here and give to MAC
      ////////////////////
      printf("\n=====================================================================\n");
...
static int init_netlink()
{
  ...
  int group = genl_ctrl_resolve_grp(nl_socket, family, "cw_broadcast");
  if(group < 0) {printf("Error resolving cw_broadcast group. Error Code: %d\n",group); retValComb |= group;}
  else{printf("cw_broadcast group is %u.\n", group);}
...
static int init_couwbat()
{
  ...
  Couwbat::SetDefaultMcs (COUWBAT_MCS_QPSK);
...
static int cwnl_couwbat_send (CouwbatMetaHeader mh, Ptr<Packet> packet)
{
  ...
  unsigned char cwnl_cmd = mh.m_flags;
  genlmsg_put(msg,NL_AUTO_PORT,NL_AUTO_SEQ,fam_id,0,0,cwnl_cmd,1);
